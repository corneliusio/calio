...
/* src/components/Day.svelte generated by Svelte v3.0.0 */
...
function instance($$self, $$props, $$invalidate) {
	...
	$$self.$$.update = ($$dirty = { selection: 1, day: 1, disabled: 1, min: 1, max: 1, mode: 1, view: 1, isDisabled: 1, isRanged: 1, isActive: 1, classes: 1 }) => {
		...
		if ($$dirty.min || $$dirty.max || $$dirty.disabled || $$dirty.view || $$dirty.mode || $$dirty.selection || $$dirty.classes) { $$invalidate('data', data = {
                min,
                max,
                disabled,
                view,
...
class Day extends SvelteComponent {
	constructor(options) {
		...
		init(this, options, instance, create_fragment, safe_not_equal, ["view", "selection", "disabled", "day", "min", "max", "mode", "state"]);
	}
...
/* src/components/Calio.svelte generated by Svelte v3.0.0 */
...
function create_each_block(ctx) {
	...
	return {
		...
		p(changed, new_ctx) {
			ctx = new_ctx;
			var day_changes = (changed.dates || changed.data) ? get_spread_update(day_spread_levels, [
...
function instance$1($$self, $$props, $$invalidate) {
    ...
    function select(day) {
        ...
        if (day) {
            ...
            switch (mode) {
                case 'range' :
                    [ selection ] = updateRange(day, selection, strict, computed.disabled); $$invalidate('selection', selection);
                    break;
                case 'multi' :
                    [ selection ] = updateMulti(day, selection, limit); $$invalidate('selection', selection);
                    break;
                default :
	...
	let computed, dates, head;
	$$self.$$.update = ($$dirty = { min: 1, max: 1, disabled: 1, computed: 1, headers: 1, view: 1, mode: 1, strict: 1, selection: 1, limit: 1, el: 1, data: 1 }) => {
		if ($$dirty.min || $$dirty.max || $$dirty.disabled) { $$invalidate('computed', computed = {
                min: makeMyDay(min),
                max: makeMyDay(max),
                disabled: new Array()
                    .concat(disabled)
		...
		if ($$dirty.computed || $$dirty.headers || $$dirty.view || $$dirty.mode || $$dirty.strict || $$dirty.selection || $$dirty.limit || $$dirty.el) { $$invalidate('data', data = {
                min: computed.min,
                max: computed.max,
		...
		if ($$dirty.el || $$dirty.data) { dispatchEvents(dispatcher, el, 'update', data); }
	};
...
class Calio extends SvelteComponent {
	constructor(options) {
		...
		init(this, options, instance$1, create_fragment$1, safe_not_equal, ["headers", "mode", "strict", "disabled", "value", "limit", "min", "max", "state", "select", "makeMyDay", "goToYear", "goToNextYear", "goToLastYear", "goToMonth", "goToNextMonth", "goToLastMonth", "goToThisMonth", "goToSelection", "goTo"]);
	}
